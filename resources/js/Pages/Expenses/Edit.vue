...
const form = useForm({
    ...
    amount_input_mode: props.expense.amount_input_mode || 'ht',
    amount_ht: props.expense.amount_ht,
    amount_ttc: props.expense.amount_ttc,
    vat_rate: parseFloat(props.expense.vat_rate),
    vat_regime: props.expense.vat_regime || 'national',
    reverse_charge_vat_rate: props.expense.reverse_charge_vat_rate !== null && props.expense.reverse_charge_vat_rate !== undefined
        ? parseFloat(props.expense.reverse_charge_vat_rate)
        : null,
...
// répartie sur plusieurs comptes. Une dépense à une seule ligne reste en saisie
// simple — elle n'a rien de « ventilé » à montrer.
const existingLines = props.expense.lines || [];
const ventilated = ref(existingLines.length > 1);

if (ventilated.value) {
    form.lines = existingLines.map((line) => ({
            ...
            <div class="overflow-x-auto rounded-2xl bg-white shadow dark:bg-surface-card">
                ...
                <div class="px-6 py-4">
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        ...
                        <div v-if="!ventilated" class="sm:col-span-2">
            ...
            <div class="overflow-x-auto rounded-2xl bg-white shadow dark:bg-surface-card">
                ...
                <div class="px-6 py-4">
                    ...
                    <div v-if="form.attachment" class="mt-3 text-sm text-slate-600 dark:text-slate-400">
                        {{ t('new_file_selected') }} {{ form.attachment.name }}
                    </div>
