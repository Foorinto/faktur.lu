...
const form = useForm({
    ...
    translation_key: props.post.translation_key || '',
    cover_image: null,
    remove_cover: false,
...
<template>
    <Head :title="t('admin_blog_head_edit', { title: post.title })" />
    ...
    <AdminLayout>
        <template #header>
            ...
            <h1 class="text-xl font-semibold text-white">{{ t('admin_blog_edit_title') }}</h1>
        </template>
                ...
                <div class="rounded-xl bg-slate-800 p-6">
                    ...
                    <div class="flex gap-2">
                        <PrimaryButton :disabled="form.processing" class="flex-1 justify-center">
                            {{ form.processing ? t('admin_saving') : t('admin_blog_update_btn') }}
                        </PrimaryButton>
                    </div>
