...
const statusTabs = computed(() => [
    ...
    {
        value: "cancelled",
        label: t("hr.leave_status_cancelled"),
...
const editRemainingDays = computed(() => {
    ...
    const emp = props.employees.find(
        (e) => e.id == editingRequest.value.employee_id,
    );
    const lt = props.leaveTypes.find((l) => l.id == editForm.leave_type_id);
    if (!emp || !lt) return null;
    const balance = emp.leave_balances?.find(
        (b) => b.leave_type_id == editForm.leave_type_id,
    );
    const baseRemaining = balance
        ? Number(balance.remaining_days)
        : Number(lt.default_days_per_year ?? 0);
    // Exclude current request from pending count (it's being edited)
    let pending = Number(
        props.pendingDays?.[editingRequest.value.employee_id]?.[
            editForm.leave_type_id
        ] ?? 0,
    );
    if (
        editingRequest.value.status === "pending" &&
        editingRequest.value.leave_type_id == editForm.leave_type_id
    ) {
        pending = Math.max(
            0,
            pending - Number(editingRequest.value.days_count),
        );
    }
...
const canSubmitEdit = computed(() => {
    return (
        ...
        editForm.start_date &&
        editForm.end_date &&
        Number(editForm.days_count) >= 0.5 &&
        !editDaysExceeded.value &&
        !editForm.processing
    );
});
...
const remainingDays = computed(() => {
    ...
    if (!emp || !lt) return null;
    const balance = emp.leave_balances?.find(
        (b) => b.leave_type_id == newForm.leave_type_id,
    );
    const baseRemaining = balance
        ? Number(balance.remaining_days)
        : Number(lt.default_days_per_year ?? 0);
    const pending = Number(
        props.pendingDays?.[newForm.employee_id]?.[newForm.leave_type_id] ?? 0,
    );
    return Math.round((baseRemaining - pending) * 10) / 10;
});
...
<template>
    ...
    <AppLayout>
        ...
        <template #header-actions>
            ...
            >
                <svg
                    class="-ml-0.5 mr-1.5 h-5 w-5 text-slate-400"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                >
        ...
        >
            ...
            >
                ...
                >
                    ...
                    <tr
                        v-for="lr in leaveRequests.data"
                        :key="lr.id"
                    ...
                    >
                        <td class="whitespace-nowrap py-4 pl-4 pr-3 sm:pl-6">
                            <div class="flex items-center">
                                <div class="h-8 w-8 flex-shrink-0">
                                    <img
                                        v-if="portraitVisible(lr.employee)"
                                        :src="route('files.employee-photo', lr.employee.id)"
                                        :alt="
                                            lr.employee?.first_name +
                                            ' ' +
                                            lr.employee?.last_name
                                        "
                                        class="h-8 w-8 rounded-lg object-cover"
                                        @error="portraitPerdu(lr.employee.id)"
                                    />
                                    <div
                                        v-else
                                        :class="[
                                    ...
                                    >
                                        <span class="text-xs font-bold">
                                            {{
                                                lr.employee?.first_name?.charAt(
                                                    0,
                                                )
                                            }}{{
                                                lr.employee?.last_name?.charAt(
                                                    0,
                                                )
                                            }}
                                        </span>
                        ...
                        <td class="whitespace-nowrap px-3 py-4 text-sm">
                            <span class="inline-flex items-center gap-1.5">
                                <span
                                    ...
                                    :style="{
                                        backgroundColor:
                                            lr.leave_type?.color || '#94a3b8',
                                    }"
                                ></span>
                                <span
                                    class="text-slate-700 dark:text-slate-300"
                                    >{{ lr.leave_type?.name }}</span
                                >
                            </span>
                        </td>
                        <td
                        ...
                        <td
                            class="whitespace-nowrap px-3 py-4 text-sm text-center text-slate-700 dark:text-slate-300"
                        >
                            {{ lr.days_count }}
                        </td>
                        <td
                        ...
                        >
                            <span
                                :class="getStatusClass(lr.status)"
                        ...
                        >
                            <div class="flex items-center justify-end gap-1">
                                ...
                                <RowAction
                                    ...
                                    icon="cancel"
                                    :label="t('hr.cancel_request')"
                                    @click="cancelRequest(lr.id)"
                                />
                                <RowAction
                                    v-if="lr.status === 'cancelled'"
                                    icon="resume"
                                    :label="t('hr.reactivate_request')"
        ...
        <!-- Pagination -->
        <div
            v-if="leaveRequests.links && leaveRequests.links.length > 3"
            ...
            <div class="text-sm text-slate-700 dark:text-slate-400">
                {{
                    t("showing_x_to_y_of_z", {
                        ...
                        to: leaveRequests.to,
                        count: leaveRequests.total,
                        items: t("hr.leave_requests").toLowerCase(),
                    })
                }}
            </div>
                ...
                >
                    <Link
                        ...
                        :class="[
                            ...
                            index === leaveRequests.links.length - 1
                                ? 'rounded-r-xl'
                                : '',
                        ]"
                        v-html="link.label"
                ...
                >
                    ...
                    <form @submit.prevent="submitNew" class="space-y-4">
                        ...
                        <div>
                            <label
                                class="block text-sm font-medium text-slate-700 dark:text-slate-300"
                                >{{ t("hr.business_days_count") }} *</label
                            >
                            <input
                                v-model.number="newForm.days_count"
                                type="number"
                                min="0.5"
                                step="0.5"
                                ...
                                class="mt-1 block w-full rounded-xl border-0 py-1.5 text-slate-900 ring-1 ring-inset ring-gray-200 focus:ring-2 focus:ring-primary-500 dark:bg-gray-800 dark:text-white dark:ring-slate-600 sm:text-sm"
                                :class="{
                                    'ring-2 ring-rose-500 dark:ring-rose-500':
                                        daysExceeded,
                                }"
                            />
                            <div
                            ...
                            <p
                                v-if="newForm.errors.days_count"
                                class="mt-1 text-xs text-rose-600"
                            >
                                {{ newForm.errors.days_count }}
                            </p>
                        </div>
                        <div>
                            <label
                        ...
                        >
                            ...
                            <button
                                ...
                                class="w-full sm:w-auto justify-center rounded-xl bg-accent-rose px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-pink-500 disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                                {{ t("hr.submit_request") }}
                            </button>
                        </div>
        ...
        <Teleport to="body">
            ...
            >
                ...
                >
                    ...
                    >
                        {{ t("hr.edit_leave_request") }}
                    </h3>
                    <p
                        v-if="editingRequest?.status === 'rejected'"
                    ...
                    >
                        {{ editingRequest?.employee?.last_name }}
                        {{ editingRequest?.employee?.first_name }}
                    </p>
                    <form @submit.prevent="submitEdit" class="space-y-4">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                min="0.5"
                                step="0.5"
                                :max="editRemainingDays ?? undefined"
                                required
                                class="mt-1 block w-full rounded-xl border-0 py-1.5 text-slate-900 ring-1 ring-inset ring-gray-200 focus:ring-2 focus:ring-primary-500 dark:bg-gray-800 dark:text-white dark:ring-slate-600 sm:text-sm"
                                :class="{
                                    'ring-2 ring-rose-500 dark:ring-rose-500':
                            ...
                            >
                                ...
                                >
                                    {{
                                        t("hr.remaining_days_info", {
                                            count: editRemainingDays,
                                        })
                                    }}
                                </span>
                                <span
                                    v-if="editDaysExceeded"
                                    class="text-xs font-semibold text-rose-600 dark:text-rose-400"
                                >
                                    {{
                                        t("hr.days_exceeded", {
                                            max: editRemainingDays,
                                        })
                                    }}
                                </span>
                            </div>
                            <p
                                v-if="editForm.errors.days_count"
                                class="mt-1 text-xs text-rose-600"
                            >
                                {{ editForm.errors.days_count }}
                            </p>
                        </div>
                        <div>
                            <label
                        ...
                        <div
                            v-if="editingRequest?.rejection_reason"
