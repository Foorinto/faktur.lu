...
const statusTabs = computed(() => {
    return [
        ...
        { value: 'active', label: t('hr.status_active'), count: props.statusCounts.active || 0 },
        { value: 'long_leave', label: t('hr.status_long_leave'), count: props.statusCounts.long_leave || 0 },
        { value: 'terminated', label: t('hr.status_terminated'), count: props.statusCounts.terminated || 0 },
    ];
});
...
const deleteEmployee = (employee) => {
    if (confirm(t('hr.confirm_delete_employee', { name: employee.full_name }))) {
...
<template>
    ...
    <AppLayout>
            ...
            <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
                <thead class="bg-slate-50 dark:bg-slate-700">
                    <tr>
                        ...
                        <th scope="col" class="hidden px-3 py-3.5 text-left text-sm font-semibold text-slate-900 dark:text-white xl:table-cell">
                            {{ t('hr.job_title') }}
                        </th>
                ...
                <tbody class="divide-y divide-slate-200 bg-white dark:divide-slate-700 dark:bg-slate-800">
                    ...
                    <tr v-for="employee in employees.data" :key="employee.id" class="hover:bg-slate-50 dark:hover:bg-slate-700">
                        ...
                        <td class="hidden whitespace-nowrap px-3 py-4 text-sm md:table-cell">
                            <span v-if="employee.department" class="inline-flex items-center gap-1.5">
                                ...
                                ></span>
                                <span class="text-slate-700 dark:text-slate-300">{{ employee.department.name }}</span>
                            </span>
                            <span v-else class="text-slate-400 dark:text-slate-500">—</span>
                        </td>
                        <td class="hidden whitespace-nowrap px-3 py-4 text-sm lg:table-cell">
                            <span
                                :class="getContractBadgeClass(employee.contract_type)"
                                class="inline-flex items-center rounded-xl px-2.5 py-0.5 text-xs font-medium"
                            >
                                {{ employee.contract_type }}
                            </span>
                        </td>
                        <td class="hidden whitespace-nowrap px-3 py-4 text-sm text-slate-500 dark:text-slate-400 xl:table-cell">
                            {{ employee.job_title || '—' }}
                        </td>
        ...
        <!-- Pagination -->
        <div v-if="employees.links && employees.links.length > 3" class="mt-6 flex items-center justify-between">
            <div class="text-sm text-slate-700 dark:text-slate-400">
                {{ t('showing_x_to_y_of_z', { from: employees.from, to: employees.to, total: employees.total, items: t('hr.employees').toLowerCase() }) }}
            </div>
                ...
                <template v-for="(link, index) in employees.links" :key="index">
                    <Link
                        ...
                        :class="[
                            ...
                            index === employees.links.length - 1 ? 'rounded-r-xl' : '',
                        ]"
                        v-html="link.label"
                        preserve-scroll
                    />
