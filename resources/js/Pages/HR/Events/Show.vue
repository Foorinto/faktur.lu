...
const deleteEvent = () => {
    ...
    router.delete(route('hr.events.destroy', props.event.id));
};

const internalParticipants = computed(() =>
    (props.event.participants || []).filter(p => p.employee)
);

const externalParticipants = computed(() =>
    (props.event.participants || []).filter(p => p.external_email && !p.employee)
);
</script>
...
<template>
    ...
    <AppLayout>
        <template #header>
            <h1 class="text-xl font-semibold text-slate-900 dark:text-white">{{ event.title }}</h1>
        </template>
        <template #header-actions>
            ...
            <button
                @click="deleteEvent"
                class="inline-flex items-center rounded-xl bg-white px-3 py-2 text-sm font-semibold text-pink-600 shadow-sm ring-1 ring-inset ring-gray-200 hover:bg-pink-50 dark:bg-gray-800 dark:ring-slate-600"
            >
                {{ t('delete') }}
            </button>
        </template>
            ...
            <div class="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 space-y-6">
                ...
                <div v-if="(internalParticipants.length + externalParticipants.length) > 0" class="bg-white dark:bg-surface-card rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                    <h2 class="text-lg font-medium text-slate-900 dark:text-white mb-4">
                        {{ t('hr_event_participants') }} ({{ internalParticipants.length + externalParticipants.length }})
                    </h2>
