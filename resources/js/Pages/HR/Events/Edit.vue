...
const submit = () => {
    form.put(route('hr.events.update', props.event.id));
};
</script>
...
<template>
    ...
    <AppLayout>
        <template #header>
            <h1 class="text-xl font-semibold text-slate-900 dark:text-white">{{ t('hr_event_edit') }}</h1>
        </template>
            ...
            <div class="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
                <form @submit.prevent="submit" class="space-y-6">
                    ...
                    <div class="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
                        <Link :href="route('hr.events.show', event.id)" class="inline-flex items-center justify-center rounded-xl border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 shadow-sm hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-slate-300">{{ t('cancel') }}</Link>
                        <PrimaryButton :disabled="form.processing">
                            {{ form.processing ? t('loading') : t('save') }}
                        </PrimaryButton>
                    </div>
                </form>
            </div>
