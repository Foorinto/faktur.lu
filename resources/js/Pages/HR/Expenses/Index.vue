...
const statusTabs = computed(() => [
    ...
    {
        value: "pending",
        label: t("hr.leave_status_pending"),
        count: props.statusCounts.pending || 0,
    },
    {
        value: "approved",
        label: t("hr.leave_status_approved"),
        count: props.statusCounts.approved || 0,
    },
    {
        value: "rejected",
        label: t("hr.leave_status_rejected"),
...
const newMaxAmountExceeded = computed(() => {
    ...
    const cat = props.categories.find(
        (c) => c.id == newForm.expense_category_id,
    );
...
const editMaxAmountExceeded = computed(() => {
    ...
    const cat = props.categories.find(
        (c) => c.id == editForm.expense_category_id,
    );
    if (!cat || !cat.max_amount) return false;
...
const deleteExistingReceipt = (receiptId) => {
    ...
    router.delete(
        route("hr.expenses.receipts.destroy", [
        ...
        ]),
        {
            preserveScroll: true,
            onSuccess: () => {
                editingExpense.value.receipts =
                    editingExpense.value.receipts.filter(
                        (r) => r.id !== receiptId,
                    );
            },
        },
    ...
    <AppLayout>
        ...
        >
            ...
            >
                ...
                >
                    ...
                    >
                        <td
                            class="whitespace-nowrap py-4 pl-4 pr-3 text-sm text-slate-700 dark:text-slate-300 sm:pl-6"
                        >
                            {{ formatDate(exp.date) }}
                        </td>
                                ...
                                <div class="h-8 w-8 flex-shrink-0">
                                    ...
                                    >
                                        <span class="text-xs font-bold">
                                            {{
                                                exp.employee?.first_name?.charAt(
                                                    0,
                                                )
                                            }}{{
                                                exp.employee?.last_name?.charAt(
                                                    0,
                                                )
                                            }}
                                        </span>
                        ...
                        <td
                            class="whitespace-nowrap px-3 py-4 text-sm text-slate-700 dark:text-slate-300"
                        >
                            {{ exp.vendor }}
                        </td>
                        <td
                            class="whitespace-nowrap px-3 py-4 text-sm text-right font-medium text-slate-900 dark:text-white"
                        >
                        ...
                        <td
                            class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm sm:pr-6"
                        >
            ...
            <div class="text-sm text-slate-700 dark:text-slate-400">
                {{
                    t("showing_x_to_y_of_z", {
                        ...
                        items: t("hr.expenses").toLowerCase(),
                    })
                }}
            </div>
        ...
        <Teleport to="body">
            ...
            >
                ...
                ></div>
                <div
                    class="relative w-full max-w-md rounded-2xl bg-white p-6 shadow-xl dark:bg-surface-card"
                >
                    <h3
                    ...
                    <form @submit.prevent="submitReject" class="space-y-4">
                        ...
                        >
                            <button
                                type="button"
                                @click="showRejectModal = false"
                                class="w-full sm:w-auto justify-center rounded-xl bg-white px-4 py-2 text-sm font-semibold text-slate-700 shadow-sm ring-1 ring-inset ring-gray-200 hover:bg-gray-50 dark:bg-gray-800 dark:text-slate-300 dark:ring-slate-600"
                            >
                                {{ t("cancel") }}
                            </button>
                            <button
                                ...
                                class="w-full sm:w-auto justify-center rounded-xl bg-rose-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-rose-500 disabled:opacity-50"
                            >
                                {{ t("hr.reject") }}
                            </button>
                        </div>
        ...
        <Teleport to="body">
            ...
            >
                ...
                ></div>
                <div
                    class="relative w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl dark:bg-surface-card my-8"
                >
                    <h3
                        class="text-lg font-medium text-slate-900 dark:text-white mb-4"
                    >
                    ...
                    <form @submit.prevent="submitNew" class="space-y-4">
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <p
                                    v-if="newForm.errors.employee_id"
                                    class="mt-1 text-xs text-rose-600"
                                >
                                    {{ newForm.errors.employee_id }}
                                </p>
                            </div>
                            <div>
                                <label
                                ...
                                >
                                <select
                                    v-model="newForm.expense_category_id"
                                    required
                                    class="mt-1 block w-full rounded-xl border-0 py-1.5 text-slate-900 ring-1 ring-inset ring-gray-200 focus:ring-2 focus:ring-primary-500 dark:bg-gray-800 dark:text-white dark:ring-slate-600 sm:text-sm"
                                >
                                    <option value="">
                                        {{ t("hr.select_category") }}
                                    </option>
                                    <option
                                ...
                                <p
                                    v-if="newForm.errors.expense_category_id"
                                    class="mt-1 text-xs text-rose-600"
                                >
                                    {{ newForm.errors.expense_category_id }}
                                </p>
                            </div>
                        </div>
                        <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            >
                            <input
                                v-model="newForm.vendor"
                                type="text"
                                required
                                class="mt-1 block w-full rounded-xl border-0 py-1.5 text-slate-900 ring-1 ring-inset ring-gray-200 focus:ring-2 focus:ring-primary-500 dark:bg-gray-800 dark:text-white dark:ring-slate-600 sm:text-sm"
                            />
                            <p
                                v-if="newForm.errors.vendor"
                                class="mt-1 text-xs text-rose-600"
                            >
                                {{ newForm.errors.vendor }}
                            </p>
                        </div>
                        <div class="grid grid-cols-3 gap-4">
                            <div>
                                <label
                                    class="block text-sm font-medium text-slate-700 dark:text-slate-300"
                                    >{{ t("hr.amount_ht") }} *</label
                                >
                                <input
                                    v-model="newForm.amount_ht"
                                    type="number"
                                    step="0.01"
                                    min="0.01"
                                    required
                                ...
                                />
                                <p
                                    v-if="newForm.errors.amount_ht"
                                    class="mt-1 text-xs text-rose-600"
                                >
                                    {{ newForm.errors.amount_ht }}
                                </p>
                            </div>
                            <div>
                                <label
                            ...
                            <div>
                                <label
                                    class="block text-sm font-medium text-slate-700 dark:text-slate-300"
                                    >{{ t("hr.amount_ttc") }}</label
                                >
                                <div
                                    class="mt-1 flex items-center rounded-xl bg-slate-50 px-3 py-1.5 text-sm font-semibold text-slate-900 ring-1 ring-inset ring-gray-200 dark:bg-gray-800 dark:text-white dark:ring-slate-600"
                                >
                                ...
                                >
                                    {{ t("hr.amount_vat") }}:
                                    {{ formatAmount(newAmountVat) }}
                                </p>
                            </div>
                        </div>
                        <div
                            v-if="newMaxAmountExceeded"
                            class="flex items-start gap-2 rounded-lg bg-amber-50 p-3 dark:bg-amber-900/20"
                        >
                            <svg
                            ...
                            >
                                {{
                                    t("hr.max_amount_exceeded", {
                                        max: formatAmount(newCategoryMaxAmount),
                                    })
                                }}
                            </p>
                        </div>
                        ...
                        <div>
                            ...
                            >
                            <div
                                v-if="newForm.receipts.length > 0"
                                class="mt-1 space-y-1.5"
                            >
                                <div
                                    v-for="(file, index) in newForm.receipts"
                                    :key="index"
                                ...
                                >
                                    <span
                                        class="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-300 truncate"
                                    >
                                        <svg
                                    ...
                                    <button
                                        ...
                                        class="rounded-lg p-1 text-slate-400 hover:bg-rose-50 hover:text-rose-600 dark:hover:bg-rose-900/20 dark:hover:text-rose-400"
                                    >
                                        <svg
                                            class="h-4 w-4"
                                            viewBox="0 0 20 20"
                            ...
                            <input
                                type="file"
                                @input="addNewReceipts"
                                multiple
                                accept=".jpg,.jpeg,.png,.pdf"
                                class="mt-1 block w-full text-sm text-slate-500 file:mr-4 file:rounded-lg file:border-0 file:bg-primary-50 file:px-4 file:py-2 file:text-sm file:font-semibold file:text-primary-700 hover:file:bg-primary-100 dark:text-slate-400 dark:file:bg-primary-900/30 dark:file:text-primary-400"
                            />
                            <p
                                v-if="newForm.errors['receipts.0']"
                                class="mt-1 text-xs text-rose-600"
                            >
                                {{ newForm.errors["receipts.0"] }}
                            </p>
                        </div>
                        <div
        ...
        <Teleport to="body">
            ...
            >
                ...
                >
                    ...
                    <p
                        v-if="editingExpense?.status === 'rejected'"
                    ...
                    >
                        {{ editingExpense?.employee?.last_name }}
                        {{ editingExpense?.employee?.first_name }}
                    </p>
                    <form @submit.prevent="submitEdit" class="space-y-4">
                        ...
                        <div class="grid grid-cols-3 gap-4">
                            ...
                            <div>
                                ...
                                >
                                    {{ t("hr.amount_vat") }}:
                                    {{ formatAmount(editAmountVat) }}
                                </p>
                            </div>
                        </div>
                        <div
                            v-if="editMaxAmountExceeded"
                            class="flex items-start gap-2 rounded-lg bg-amber-50 p-3 dark:bg-amber-900/20"
                        >
                            <svg
                                class="h-5 w-5 flex-shrink-0 text-amber-500 mt-0.5"
                        ...
                        <div>
                            ...
                            >
                            <div
                                v-if="editingExpense?.receipts?.length > 0"
                                class="mt-1 space-y-1.5"
                            >
                                <div
                                    v-for="receipt in editingExpense.receipts"
                                    :key="receipt.id"
                                    class="flex items-center justify-between rounded-lg bg-slate-50 px-3 py-2 ring-1 ring-inset ring-gray-200 dark:bg-gray-800 dark:ring-slate-600"
                                >
                                    <a
                                    ...
                                    <button
                                        type="button"
                                        @click="
                                            deleteExistingReceipt(receipt.id)
                                        "
                                        class="rounded-lg p-1 text-slate-400 hover:bg-rose-50 hover:text-rose-600 dark:hover:bg-rose-900/20 dark:hover:text-rose-400"
                                        :title="t('hr.remove_receipt')"
                                    >
                                        <svg
                                            class="h-4 w-4"
                                            viewBox="0 0 20 20"
                            ...
                            <!-- New files to add -->
                            <div
                                v-if="editForm.receipts.length > 0"
                                class="mt-1.5 space-y-1.5"
                            >
                                <div
                                    v-for="(file, index) in editForm.receipts"
                                    :key="'new-' + index"
                                    class="flex items-center justify-between rounded-lg bg-emerald-50 px-3 py-2 ring-1 ring-inset ring-emerald-200 dark:bg-emerald-900/20 dark:ring-emerald-800"
                                >
                                    <span
                                        class="flex items-center gap-2 text-sm text-emerald-700 dark:text-emerald-400 truncate"
                                    >
                                        <svg
                                            class="h-4 w-4 flex-shrink-0"
                                            viewBox="0 0 20 20"
                                    ...
                                    <button
                                        type="button"
                                        @click="removeEditReceipt(index)"
                                        class="rounded-lg p-1 text-emerald-400 hover:bg-rose-50 hover:text-rose-600 dark:hover:bg-rose-900/20 dark:hover:text-rose-400"
                                    >
                                        <svg
                                            class="h-4 w-4"
                                            viewBox="0 0 20 20"
                            ...
                            <input
                                type="file"
                                @input="addEditReceipts"
                                multiple
                                accept=".jpg,.jpeg,.png,.pdf"
                                class="mt-1.5 block w-full text-sm text-slate-500 file:mr-4 file:rounded-lg file:border-0 file:bg-primary-50 file:px-4 file:py-2 file:text-sm file:font-semibold file:text-primary-700 hover:file:bg-primary-100 dark:text-slate-400 dark:file:bg-primary-900/30 dark:file:text-primary-400"
                            />
                        </div>
                        <div
                            v-if="editingExpense?.rejection_reason"
                            class="rounded-lg bg-rose-50 p-3 dark:bg-rose-900/20"
                        >
                            <p
                                class="text-xs font-medium text-rose-700 dark:text-rose-400"
                            >
                                {{ t("hr.previous_rejection_reason") }}
                            </p>
                            <p
                                class="mt-1 text-sm text-rose-600 dark:text-rose-300"
                            >
                                {{ editingExpense.rejection_reason }}
                            </p>
                        </div>
                        <div
                            class="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end pt-2"
