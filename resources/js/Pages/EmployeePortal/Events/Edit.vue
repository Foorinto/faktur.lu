...
const form = useForm({
    ...
    video_url: props.event.video_url || '',
    description: props.event.description || '',
    participant_employee_ids: (props.event.participants || []).filter(p => p.employee_id).map(p => p.employee_id),
    participant_external_emails: (props.event.participants || []).filter(p => p.external_email && !p.employee_id).map(p => p.external_email),
});
...
const submit = () => form.put(route('employee-portal.shared-calendar.events.update', props.event.id));
</script>

<template>
    <Head :title="t('hr_event_edit')" />

    <EmployeePortalLayout>
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white mb-6">{{ t('hr_event_edit') }}</h1>
            ...
            <form @submit.prevent="submit" class="space-y-6">
                <div class="bg-white dark:bg-surface-card rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 space-y-4">
                    <div>
                        <InputLabel :value="t('hr_event_title')" />
                        <input v-model="form.title" type="text" required maxlength="200" class="mt-1 block w-full rounded-xl border-gray-200 shadow-sm focus:border-primary-500 focus:ring-primary-500 dark:border-gray-700 dark:bg-gray-800 dark:text-white" />
                        <InputError :message="form.errors.title" class="mt-2" />
                    </div>
                    <div>
                        <InputLabel :value="t('hr_event_type')" />
                ...
                <div class="bg-white dark:bg-surface-card rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 space-y-4">
                    ...
                    <div v-if="form.location_type === 'address'">
                        <input v-model="form.address" type="text" maxlength="500" class="mt-1 block w-full rounded-xl border-gray-200 shadow-sm focus:border-primary-500 focus:ring-primary-500 dark:border-gray-700 dark:bg-gray-800 dark:text-white" />
                    </div>
                    <div v-if="form.location_type === 'video'">
                        <input v-model="form.video_url" type="url" maxlength="500" class="mt-1 block w-full rounded-xl border-gray-200 shadow-sm focus:border-primary-500 focus:ring-primary-500 dark:border-gray-700 dark:bg-gray-800 dark:text-white" />
                    </div>
                </div>
                ...
                <div class="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
                    <Link :href="route('employee-portal.shared-calendar.events.show', event.id)" class="inline-flex items-center justify-center rounded-xl border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 shadow-sm hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-slate-300">{{ t('cancel') }}</Link>
