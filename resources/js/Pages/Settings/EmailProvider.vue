            ...
            <div v-if="needsConfiguration" class="overflow-hidden rounded-2xl bg-white shadow-xl shadow-slate-200/50 border border-slate-200 dark:bg-slate-800 dark:border-slate-700 dark:shadow-slate-900/50">
                ...
                <div class="px-6 py-4 space-y-4">
                    <div v-for="(fieldConfig, field) in currentConfigFields" :key="field">
                        <label :for="field" class="block text-sm font-medium text-slate-700 dark:text-slate-300">
                            {{ fieldConfig.label }}
                            <span v-if="fieldConfig.required" class="text-pink-500">*</span>
                        </label>
                        ...
                        <div v-else-if="fieldConfig.type === 'password'" class="relative mt-1">
                            ...
                            <button
                                ...
                                class="absolute inset-y-0 right-0 flex items-center pr-3 text-slate-400 hover:text-slate-500"
                            >
                        ...
                        <!-- Help text -->
                        <p v-if="fieldConfig.help" class="mt-1 text-xs text-slate-500 dark:text-slate-400">
                            {{ fieldConfig.help }}
                        </p>
                    </div>

                    <p v-if="form.errors.config" class="text-sm text-pink-600">
                        {{ form.errors.config }}
                    </p>
                </div>
            </div>
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-3">
                            <div
                                ...
                                :class="settings.provider_verified ? 'bg-emerald-100 dark:bg-emerald-900' : 'bg-amber-100 dark:bg-amber-900'"
                            >
                                <svg v-if="settings.provider_verified" class="w-6 h-6 text-emerald-600 dark:text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                                </svg>
                                <svg v-else class="w-6 h-6 text-amber-600 dark:text-amber-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
